  ...
  for (const event of events) {
    try {
      ...
      if (attempts >= maxAttempts) {
        ...
        failedCount++;
        logger
          .child("outbox-processor")
          .error(`[OutboxProcessor] Event ${event.id} failed permanently after ${attempts} attempts`);
      } else {
        ...
        if (!urlCheck.valid) {
          ...
          await storage
            .createOutboundWebhookLog({
              ...
              payload: redactDeliveryLog(event.payload) as Record<string, unknown>,
              responseStatus: 0,
