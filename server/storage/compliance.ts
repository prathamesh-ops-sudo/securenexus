...
export async function getPolicyCheckForOrg(id: string, orgId: string): Promise<PolicyCheck | undefined> {
  const [check] = await db
    .select()
    .from(policyChecks)
    .where(and(eq(policyChecks.id, id), eq(policyChecks.orgId, orgId)));
  return check;
}
