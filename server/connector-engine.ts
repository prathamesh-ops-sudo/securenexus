...
function normalizeBatch(
  rawAlerts: any[],
  normalizer: (raw: any) => Partial<InsertAlert>,
  orgId: string | null,
  ...
  for (let i = 0; i < rawAlerts.length; i += BATCH_SIZE) {
    ...
    for (const raw of batch) {
      try {
        ...
        failed++;
        errors.push(`Normalization failed: ${err.message}`);
      }
    }
  }
...
export async function syncConnector(connector: Connector): Promise<SyncResult> {
  ...
  try {
    ...
    const { normalized, failed, errors } = normalizeBatch(rawAlerts, normalizer, connector.orgId);
