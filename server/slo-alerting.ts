...
const DEFAULT_SLO_TARGETS: Array<{
  ...
  { service: "api", metric: "latency_p95", target: 500, operator: "lte", windowMinutes: 15, description: "API p95 latency must stay under 500ms" },
  { service: "api", metric: "error_rate", target: 1, operator: "lte", windowMinutes: 15, description: "API error rate must stay under 1%" },
  { service: "api", metric: "availability", target: 99.5, operator: "gte", windowMinutes: 60, description: "API availability must exceed 99.5%" },
  { service: "ingestion", metric: "latency_p95", target: 2000, operator: "lte", windowMinutes: 15, description: "Ingestion p95 latency must stay under 2000ms" },
  { service: "ingestion", metric: "error_rate", target: 0.5, operator: "lte", windowMinutes: 15, description: "Ingestion error rate must stay under 0.5%" },
  { service: "ai", metric: "latency_p95", target: 5000, operator: "lte", windowMinutes: 30, description: "AI inference p95 latency must stay under 5000ms" },
  { service: "ai", metric: "error_rate", target: 5, operator: "lte", windowMinutes: 30, description: "AI error rate must stay under 5%" },
  { service: "connector", metric: "latency_p95", target: 10000, operator: "lte", windowMinutes: 30, description: "Connector sync p95 latency must stay under 10s" },
  { service: "connector", metric: "error_rate", target: 2, operator: "lte", windowMinutes: 30, description: "Connector error rate must stay under 2%" },
  { service: "enrichment", metric: "latency_p95", target: 3000, operator: "lte", windowMinutes: 30, description: "Enrichment p95 latency must stay under 3000ms" },
];
...
export async function evaluateAndAlert(): Promise<{
  ...
  for (const evaluation of evaluations) {
    ...
    const operatorLabel = evaluation.metric === "availability" || evaluation.metric === "throughput" ? "below" : "above";
    ...
    const breach: SloBreachRecord = {
      ...
      severity,
      message: `SLO breach: ${evaluation.service}/${evaluation.metric}${evaluation.endpoint && evaluation.endpoint !== "*" ? ` ${evaluation.endpoint}` : ""} is ${evaluation.actual} (${operatorLabel} target ${evaluation.target}). ${evaluation.description || ""}`.trim(),
      detectedAt: new Date(),
    };
...
export async function getBreachHistory(service?: string, hoursBack: number = 24): Promise<Array<{
  service: string;
  metric: string;
  endpoint: string;
  value: number;
