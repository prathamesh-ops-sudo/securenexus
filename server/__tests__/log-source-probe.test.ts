...
import { buildHttpPushProbeResult, buildReceiverProbeResult, mapCloudWatchProbeError } from "../log-source-probe";
...
describe("log source probe result mapping", () => {
  ...
  it.each(["syslog", "journald", "windows_event_log"])(
    "reports persisted receipt evidence for receiver-side %s sources",
    (sourceType) => {
      const result = buildReceiverProbeResult({
        sourceType,
        eventsReceived: 1200,
        lastEventAt: "2026-04-01T00:00:00.000Z",
      });
