...
export function registerFindingLineageRoutes(app: Express): void {
  ...
  app.post("/api/finding-lineage/filter", isAuthenticated, async (req, res) => {
    try {
      ...
      if (body.minRiskScore !== undefined) findings = findings.filter((f) => (f.riskScore ?? 0) >= body.minRiskScore!);
      if (body.maxRiskScore !== undefined) findings = findings.filter((f) => (f.riskScore ?? 0) <= body.maxRiskScore!);
