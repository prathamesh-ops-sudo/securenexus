...
export function registerSecurityGraphRoutes(app: Express): void {
      ...
      try {
        ...
        if (body.relationshipTypes) {
          for (const t of body.relationshipTypes) {
            if (!validateRelationshipType(t))
              return res.status(400).json({ message: `Invalid relationship type: ${t}` });
          }
        }
  ...
  // Delete relationship
  app.delete(
    "/api/security-graph/relationships/:id",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("analyst"),
