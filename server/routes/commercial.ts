...
export function registerCommercialRoutes(app: Express): void {
  ...
  app.get("/api/usage-metering", isAuthenticated, resolveOrgContext, requireOrgId, async (req, res) => {
    try {
      ...
      const metrics = [
        ...
        {
          type: "connectors_active",
          label: "Active Connectors",
          current: connectorsActive,
          limit: plan.maxConnectors,
          unit: "connectors",
        },
        {
          type: "ai_tokens_used",
        ...
        },
        {
          ...
          limit: plan.automationRunsPerMonth,
          unit: "runs",
        },
      ];
      ...
      const metricsWithPct = metrics.map((m) => ({
        ...
        status:
          m.limit > 0 && (m.current / m.limit) * 100 >= plan.hardThresholdPct
            ? "critical"
            : m.limit > 0 && (m.current / m.limit) * 100 >= plan.softThresholdPct
              ? "warning"
              : "ok",
      }));
      ...
      res.status(500).json({ message: "Failed to fetch usage metering" });
    }
  });
  ...
  app.get("/api/onboarding-checklist", isAuthenticated, resolveOrgContext, requireOrgId, async (req, res) => {
    try {
      ...
      if (steps.length === 0) {
        const defaultSteps = [
          {
            stepKey: "first_connector",
            stepLabel: "Connect Your First Data Source",
            stepDescription: "Set up a connector to start ingesting security events",
            category: "setup",
            sortOrder: 1,
            targetUrl: "/connectors",
          },
          {
            stepKey: "first_alert_review",
            stepLabel: "Review Your First Alert",
            stepDescription: "Triage an incoming alert and set its status",
            category: "setup",
            sortOrder: 2,
            targetUrl: "/alerts",
          },
          {
            stepKey: "first_incident",
            stepLabel: "Create Your First Incident",
            stepDescription: "Escalate an alert or create a new incident for investigation",
            category: "setup",
            sortOrder: 3,
            targetUrl: "/incidents",
          },
          {
            stepKey: "first_playbook",
            stepLabel: "Run Your First Playbook",
            stepDescription: "Execute an automated response playbook",
            category: "automation",
            sortOrder: 4,
            targetUrl: "/playbooks",
          },
          {
            ...
            stepDescription: "Add team members to collaborate on investigations",
            category: "team",
            sortOrder: 5,
            targetUrl: "/team",
          },
          {
            stepKey: "configure_notifications",
            stepLabel: "Set Up Notifications",
            stepDescription: "Configure alert notification channels (email, Slack, webhook)",
          ...
          {
            stepKey: "enable_ai",
            stepLabel: "Enable AI Triage",
            stepDescription: "Let the AI engine auto-classify and prioritize alerts",
            category: "ai",
            sortOrder: 7,
            targetUrl: "/ai-engine",
          },
          {
            stepKey: "setup_compliance",
            stepLabel: "Configure Compliance Policies",
            stepDescription: "Set data retention, DSAR workflows, and audit settings",
            category: "governance",
            sortOrder: 8,
  ...
  app.post(
    "/api/onboarding-checklist/:stepKey/complete",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    async (req, res) => {
  ...
  app.get("/api/workspace-templates", isAuthenticated, async (req, res) => {
    try {
        ...
        const defaults = [
          {
            ...
            icon: "Shield",
            description:
              "Streamlined security operations for small and medium businesses. Includes essential connectors, basic playbooks, and email notifications.",
            ...
            playbooksConfig: [
              {
                ...
                steps: ["isolate_email", "block_sender", "notify_user"],
              },
              {
                name: "Malware Alert",
                trigger: "alert.category=malware",
                steps: ["quarantine_file", "scan_endpoint", "create_incident"],
              },
            ],
          ...
          },
          {
            ...
            icon: "Building",
            description:
              "Full-scale security operations center for large organizations. Multi-tier RBAC, advanced playbooks, SIEM/SOAR/TIP integrations, and compliance automation.",
            ...
            playbooksConfig: [
              {
                ...
                steps: ["create_incident", "page_oncall", "start_investigation"],
              },
              {
                name: "Lateral Movement",
                trigger: "alert.tactic=lateral_movement",
                steps: ["isolate_host", "block_ip", "forensic_snapshot"],
              },
              {
                name: "Data Exfiltration",
                trigger: "alert.tactic=exfiltration",
                steps: ["block_destination", "revoke_tokens", "legal_hold"],
              },
            ],
            notificationConfig: [
          ...
          },
          {
            ...
            icon: "Cloud",
            description:
              "Designed for cloud-native organizations. Focus on CSPM, container security, IAM monitoring, and serverless threat detection.",
            ...
            playbooksConfig: [
              {
                ...
                trigger: "cspm.finding=public_bucket",
                steps: ["restrict_bucket_acl", "notify_owner", "create_incident"],
              },
              {
                name: "IAM Key Compromise",
                trigger: "alert.category=credential_compromise",
                steps: ["revoke_key", "rotate_secrets", "audit_usage"],
              },
              {
                name: "Container Escape",
                trigger: "alert.category=container_escape",
                steps: ["kill_pod", "cordon_node", "forensic_snapshot"],
              },
            ],
            notificationConfig: [
      ...
      res.status(500).json({ message: "Failed to fetch workspace templates" });
    }
  });
  ...
  app.post(
    "/api/workspace-templates/:id/apply",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("admin"),
    async (req, res) => {
      try {
        ...
        for (const pb of playbooksConfig) {
          try {
            await storage.createPlaybook({
              ...
              actions: pb.steps
                ? pb.steps.map((s: string, i: number) => ({ id: `step-${i}`, action: s, order: i }))
                : [],
              status: "inactive",
            });
        ...
        res.status(500).json({ message: "Failed to apply workspace template" });
      }
    },
  );
}
