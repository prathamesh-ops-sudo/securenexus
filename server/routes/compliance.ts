...
import {
  ...
  insertEvidenceLockerItemSchema,
...
export function registerComplianceRoutes(app: Express): void {
  ...
  app.post(
    "/api/compliance/dsar/:id/fulfill",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("admin"),
    async (req, res) => {
      try {
        ...
        await storage.createAuditLog({
          ...
          resourceId: request.id,
          details: {
            alertsFound: summary.alertsFound,
            entitiesFound: summary.entitiesFound,
            auditLogsFound: summary.auditLogsFound,
          },
        });

        res.json({
          request: { ...request, status: "fulfilled", fulfilledAt: new Date(), resultSummary: summary },
          summary,
        });
      } catch (error) {
      ...
      if (reportType === "gdpr_article30") {
        ...
        const [oldestAlertRow] = orgId
          ? await database
              .select()
              .from(alertsTable)
              .where(eqOp(alertsTable.orgId, orgId))
              .orderBy(ascOp(alertsTable.createdAt))
              .limit(1)
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          results: orgResult || { message: "No retention policy configured for this organization" },
        });
      } catch (error) {
  ...
  app.get("/api/suppression-rules/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (rule.orgId && user?.orgId && rule.orgId !== user.orgId)
  ...
  app.get("/api/sla-policies/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (policy.orgId && user?.orgId && policy.orgId !== user.orgId)
        return res.status(403).json({ message: "Access denied" });
      res.json(policy);
    } catch (error) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!parsed.success)
          return res.status(400).json({ message: "Invalid control data", errors: parsed.error.flatten() });
      ...
      try {
        const seedControls = [
          ...
          },
          {
            framework: "NIST CSF",
            controlId: "ID.AM-2",
            title: "Asset Management - Software Platforms",
          ...
          },
          {
            ...
            description: "Policies for information security are reviewed at planned intervals",
            category: "Policy",
          },
          {
            framework: "ISO 27001",
            controlId: "A.6.1",
            title: "Organization of Security",
          ...
          },
          {
            ...
            description: "Prior to employment security measures",
          ...
          },
          {
            ...
            description: "Establish and maintain secure configurations for hardware and software",
            category: "Basic",
          },
          {
            framework: "CIS",
          ...
          },
          {
            framework: "CIS",
            controlId: "CIS-5",
            title: "Controlled Use of Admin Privileges",
            description: "Manage the controlled use of administrative privileges",
            category: "Basic",
          },
          {
            framework: "CIS",
            controlId: "CIS-6",
            title: "Maintenance and Analysis of Audit Logs",
  ...
  app.patch(
    "/api/compliance-control-mappings/:id",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("admin"),
  ...
  app.get("/api/legal-holds", isAuthenticated, resolveOrgContext, requireOrgId, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch legal holds" });
    }
  });
  ...
  app.get("/api/legal-holds/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch legal hold" });
    }
  });
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res.status(500).json({ message: "Failed to create legal hold" });
      }
    },
  );
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        res.status(500).json({ message: "Failed to update legal hold" });
      }
    },
  );

  app.post(
    "/api/legal-holds/:id/deactivate",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("admin"),
    async (req, res) => {
      try {
        ...
        res.status(500).json({ message: "Failed to deactivate legal hold" });
      }
    },
  );
