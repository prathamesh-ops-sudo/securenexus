...
export function registerPlaybooksExecutionRoutes(app: Express): void {
  ...
  app.post(
    "/api/playbook-executions/:id/resume",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("analyst"),
  ...
  app.post(
    "/api/playbook-executions/:id/rollback",
    isAuthenticated,
    resolveOrgContext,
    requireOrgId,
    requireMinRole("analyst"),
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const running = allExecs.filter((e: any) => e.status === "running");
        const awaitingApproval = allExecs.filter((e: any) => e.status === "awaiting_approval");
