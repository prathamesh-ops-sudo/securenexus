...
export function registerPamRoutes(app: Express): void {
  ...
  app.post("/api/pam/sessions", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const {
        requesterName,
      ...
      if (targetAccount === "root" || targetAccount === "Administrator") riskFactors.push("Root/admin target account");
  ...
  // Approve PAM session
  app.post("/api/pam/sessions/:id/approve", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const id = String(req.params.id);
  ...
  // Deny PAM session
  app.post("/api/pam/sessions/:id/deny", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const id = String(req.params.id);
  ...
  app.post("/api/pam/sessions/:id/activate", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const id = String(req.params.id);
  ...
  app.post("/api/pam/sessions/:id/terminate", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const id = String(req.params.id);
      const { reason } = req.body;
  ...
  app.post("/api/pam/sessions/:id/complete", isAuthenticated, async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const id = String(req.params.id);
