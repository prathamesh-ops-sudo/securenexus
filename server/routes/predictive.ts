      ...
      const forecasts = [
        {
          ...
          reasoning: "Historical pattern shows consistent phishing attempts. Employee training completion rate at 65%.",
          indicators: [
    ...
    try {
      ...
      const surfaceItems = Object.values(assetMap).map((item: any) => {
        ...
        if (item.categories.has("lateral_movement")) {
          vulns.push({ type: "lateral_movement", severity: "high", description: "Lateral movement patterns observed" });
        }
    ...
    try {
      ...
      if (lateralMovement.length > 2) {
        recommendations.push({
          ...
          reasoning: "Network segmentation limits blast radius and prevents lateral movement between critical assets.",
      ...
      if (openIncidents.length > 5) {
        recommendations.push({
          ...
          reasoning: "Security Orchestration, Automation and Response reduces MTTR by 95% and analyst workload by 50%.",
          estimatedEffort: "high",
          estimatedImpact: "high",
  ...
  app.post("/api/predictive/recompute", requireAuth, async (req, res) => {
    try {
      ...
      return res.json({
        ...
        modelsAffected: ["attack_forecasting", "anomaly_detection", "attack_surface_analysis", "recommendation_engine"],
      });
    } catch (error: any) {
