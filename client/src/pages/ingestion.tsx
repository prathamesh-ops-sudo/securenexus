...
export default function IngestionPage() {
  ...
  const { data: apiKeysData, isLoading: keysLoading } = useQuery<ApiKeyDisplay[]>({
    queryKey: ["/api/api-keys"],
  });

  const { data: ingestionLogs, isLoading: logsLoading } = useQuery<IngestionLog[]>({
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "success": return <Badge variant="default" data-testid={`badge-status-${status}`}><CheckCircle className="h-3 w-3 mr-1" /> Success</Badge>;
      case "deduped": return <Badge variant="secondary" data-testid={`badge-status-${status}`}><Copy className="h-3 w-3 mr-1" /> Deduped</Badge>;
      case "partial": return <Badge variant="secondary" data-testid={`badge-status-${status}`}><AlertTriangle className="h-3 w-3 mr-1" /> Partial</Badge>;
      case "failed": return <Badge variant="destructive" data-testid={`badge-status-${status}`}><XCircle className="h-3 w-3 mr-1" /> Failed</Badge>;
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" data-testid="text-page-title"><span className="gradient-text-red">Alert Ingestion</span></h1>
          <p className="text-sm text-muted-foreground">Manage API keys and monitor alert ingestion from security tools</p>
                  ...
                  <Card key={key} className="bg-muted/30">
                    <CardContent className="p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium truncate" data-testid={`text-source-name-${key}`}>{name}</span>
                      </div>
                      {sourceStats ? (
                ...
                <TableBody>
                  {ingestionLogs.map((log) => (
                    <TableRow key={log.id} data-testid={`row-ingestion-log-${log.id}`}>
                      <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{formatTime(log.receivedAt)}</TableCell>
                      ...
                      <TableCell className="text-right text-sm text-muted-foreground">{log.processingTimeMs ?? "-"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
  ...
  -d '[{"sid": "event1", "severity": "high"}, {"sid": "event2", "severity": "low"}]'`}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {sourcesData && Object.entries(sourcesData.sourceNames).map(([key, name]) => (
                <Badge key={key} variant="outline" data-testid={`badge-source-${key}`}>{name}</Badge>
              ))}
            </div>
          </div>
        </CardContent>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            >
              {createKeyMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Key className="h-4 w-4 mr-1" />}
              Create Key
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!createdKey} onOpenChange={() => setCreatedKey(null)}>
        <DialogContent>
          ...
          {createdKey && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Button size="icon" variant="ghost" onClick={() => copyToClipboard(createdKey.key)} data-testid="button-copy-key">
          ...
          <DialogFooter>
            <Button onClick={() => { setCreatedKey(null); setShowKey(false); }} data-testid="button-done-key">Done</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
