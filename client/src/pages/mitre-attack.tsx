...
import {
  Shield, Crosshair, Grid3X3, AlertTriangle, Clock, Target,
} from "lucide-react";
...
export default function MitreAttackPage() {
  ...
  const { techniqueMap, tacticTechniques, maxCount, topTechnique, allTechniques } = useMemo(() => {
    ...
    if (!alerts) return { techniqueMap: tMap, tacticTechniques: tTechniques, maxCount: 0, topTechnique: "N/A", allTechniques: [] as TechniqueData[] };
    ...
    for (const alert of alerts) {
      ...
      if (existing) {
        ...
        const alertDate = alert.detectedAt ? new Date(alert.detectedAt) : alert.createdAt ? new Date(alert.createdAt) : null;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto" data-testid="page-mitre-attack">
      <div>
        <h1 className="text-2xl font-bold tracking-tight" data-testid="text-page-title"><span className="gradient-text-red">MITRE ATT&CK Coverage</span></h1>
        ...
        <CardContent>
          {!hasData ? (
            <div className="flex flex-col items-center justify-center py-12 text-center" data-testid="empty-state">
              ...
              <p className="text-xs text-muted-foreground mt-1">Alerts with MITRE tactic and technique mappings will appear here</p>
            </div>
          ) : (
            <div className="overflow-x-auto -mx-4 px-4 pb-2" data-testid="matrix-scroll-container">
              <div className="flex gap-2" style={{ minWidth: `${MITRE_TACTICS.length * 140}px` }}>
                {MITRE_TACTICS.map((tactic) => {
                  ...
                  return (
                    <div key={tactic} className="flex-1 min-w-[130px]" data-testid={`tactic-column-${tactic.toLowerCase().replace(/\s+/g, "-")}`}>
                      <div className="mb-2">
                        <Badge variant="outline" className="text-[10px] w-full justify-center text-center whitespace-normal leading-tight py-1" data-testid={`badge-tactic-${tactic.toLowerCase().replace(/\s+/g, "-")}`}>
                          {tactic}
                        </Badge>
                      </div>
                      <div className="space-y-1">
                        {techniques.length === 0 ? (
                          <div className="text-[10px] text-muted-foreground text-center py-3 rounded-md bg-muted/30" data-testid={`empty-tactic-${tactic.toLowerCase().replace(/\s+/g, "-")}`}>
                            No detections
                          </div>
                        ) : (
                          techniques.map((tech) => (
                            ...
                            >
                              ...
                              <div className="text-muted-foreground tabular-nums">{tech.count} alert{tech.count !== 1 ? "s" : ""}</div>
                            </div>
                          ))
                        )}
                      </div>
              ...
              <table className="w-full text-sm" data-testid="table-techniques">
                ...
                <tbody>
                  {allTechniques.map((tech, i) => {
                    ...
                    return (
                      <tr key={`${tech.tactic}-${tech.technique}`} className="border-b border-border/50" data-testid={`row-technique-${i}`}>
                        ...
                        <td className="py-2 px-3">
                          <Badge variant={severityVariant(severity)} className="text-[10px] capitalize" data-testid={`badge-severity-${i}`}>
                            {severity}
                          </Badge>
                        </td>
                        <td className="py-2 px-3 text-muted-foreground">
                          <div className="flex items-center gap-1">
                            ...
                            {tech.lastSeen ? tech.lastSeen.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "Unknown"}
                          </div>
                        </td>
                      </tr>
                    );
