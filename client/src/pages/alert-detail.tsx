...
export default function AlertDetailPage() {
  ...
  const { data: alertEntities } = useQuery<
    {
      id: string;
      type: string;
      value: string;
  ...
  if (!alert) {
    return (
      <div className="p-4 md:p-6 text-center py-20">
        ...
        <Link href="/alerts">
          <Button variant="outline" className="mt-4" data-testid="button-back-alerts">
            Back to Alerts
          </Button>
        </Link>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <div className="font-medium" data-testid="text-alert-source">
                    {alert.source}
                  </div>
                </div>
                <div>
                  <span className="text-xs text-muted-foreground">Category</span>
                  <div className="font-medium capitalize" data-testid="text-alert-category">
                    {alert.category?.replace(/_/g, " ") || "Other"}
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="text-xs" data-testid="text-detected-at">
                    {formatTimestamp(alert.detectedAt)}
                  </div>
                </div>
                <div>
                  <span className="text-xs text-muted-foreground">Ingested At</span>
                  <div className="text-xs" data-testid="text-ingested-at">
                ...
                {alert.sourceEventId && (
                  <div className="col-span-2">
                    ...
                    <div className="text-xs font-mono" data-testid="text-source-event-id">
                      {alert.sourceEventId}
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <span
                        ...
                        data-testid="text-mitre-technique"
                      >
                        {alert.mitreTechnique}
                      </span>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {alert.sourceIp && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-source-ip">
                        {alert.sourceIp}
                      </div>
                    </div>
                  )}
                  {alert.destIp && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-dest-ip">
                        {alert.destIp}
                      </div>
                    </div>
                  )}
                  {alert.sourcePort != null && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-source-port">
                        {alert.sourcePort}
                      </div>
                    </div>
                  )}
                  {alert.destPort != null && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-dest-port">
                        {alert.destPort}
                      </div>
                    </div>
                  )}
                  {alert.protocol && (
                    <div>
                      ...
                      <div className="text-xs" data-testid="text-protocol">
                        {alert.protocol}
                      </div>
                    </div>
                  )}
                  {alert.hostname && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-hostname">
                        {alert.hostname}
                      </div>
                    </div>
                  )}
                  {alert.domain && (
                    <div>
                      ...
                      <div className="font-mono text-xs" data-testid="text-domain">
                        {alert.domain}
                      </div>
                    </div>
                  )}
                  {alert.url && (
                    <div className="col-span-2">
                      ...
                      <div className="font-mono text-xs break-all" data-testid="text-url">
                        {alert.url}
                      </div>
                    </div>
                  )}
                  {alert.fileHash && (
                    <div className="col-span-2">
                      ...
                      <div className="font-mono text-xs break-all" data-testid="text-file-hash">
                        {alert.fileHash}
                      </div>
                    </div>
                  )}
          ...
          {hasCorrelation &&
            (() => {
              ...
              const scoreColor =
                score != null ? (score > 70 ? "bg-green-500" : score >= 40 ? "bg-yellow-500" : "bg-red-500") : "";
              ...
              while ((sevMatch = severityPattern.exec(reasonFull)) !== null) {
                severities.push(
                  ...sevMatch[1]
                    .split(",")
                    .map((s) => s.trim())
                    .filter((s) => ["critical", "high", "medium", "low"].includes(s.toLowerCase())),
                );
              }
                  ...
                  <CardContent className="space-y-4">
                    {score != null && (
                      <div>
                        <div className="flex items-center justify-between gap-2 mb-1">
                          ...
                          <span className="text-xs font-semibold" data-testid="text-confidence-score">
                            {Math.round(score)}%
                          </span>
                        </div>
                    ...
                    {method && (
                      <div>
                        ...
                        <div className="text-sm font-medium mt-0.5" data-testid="text-correlation-method">
                          {method}
                        </div>
                      </div>
                    )}
                    ...
                    {reasonLines.length > 0 && (
                      <div>
                        ...
                        <pre
                          className="text-xs font-mono bg-muted/30 p-3 rounded-md mt-1 whitespace-pre-wrap break-words overflow-auto max-h-48"
                          data-testid="text-correlation-reason"
                        >
                          {reasonLines.join("\n")}
                        </pre>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {sharedEntities.map((entity, i) => (
                            <Badge
                              ...
                              variant="secondary"
                              className="text-[10px]"
                              data-testid={`badge-shared-entity-${i}`}
                            >
                              {entity}
                            </Badge>
                          ))}
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {tactics.map((tactic, i) => (
                            <Badge
                              ...
                              data-testid={`badge-mitre-tactic-${i}`}
                            >
                              {tactic}
                            </Badge>
                          ))}
                    ...
                    {alert.correlationClusterId && (
                      <div>
                        ...
                        <div className="text-xs font-mono mt-0.5" data-testid="text-cluster-id">
                          {alert.correlationClusterId}
                        </div>
                      </div>
                    )}
                ...
                <CardTitle className="text-sm font-semibold flex items-center gap-2 flex-wrap">
                  ...
                  {triageResult.threatIntelSources && triageResult.threatIntelSources.length > 0 && (
                    <Badge
                      ...
                      data-testid="badge-threat-intel-enriched"
                    >
              ...
              <CardContent className="space-y-3" data-testid="triage-result">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs" data-testid="text-triage-category">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {triageResult.mitreTactic && (
                    <span
                      ...
                      data-testid="text-triage-mitre-tactic"
                    >
                      {triageResult.mitreTactic}
                    </span>
                  )}
                  {triageResult.mitreTechnique && (
                    <span
                      ...
                      data-testid="text-triage-mitre-technique"
                    >
                      {triageResult.mitreTechnique}
                    </span>
                  )}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {triageResult.relatedIocs.map((ioc, i) => (
                        <span
                          key={i}
                          className="px-1.5 py-0.5 rounded bg-muted text-[10px] font-mono"
                          data-testid={`triage-ioc-${i}`}
                        >
                          {typeof ioc === "string" ? ioc : `${ioc.value} (${ioc.type})`}
                        </span>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1" data-testid="triage-intel-sources">
                      {triageResult.threatIntelSources.map((source, i) => (
                        <Badge
                          ...
                          data-testid={`badge-intel-source-${i}`}
                        >
                          <Shield className="h-3 w-3" />
                          {source}
                        </Badge>
                  ...
                  <div className="space-y-3 pt-2 border-t">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => submitFeedback.mutate({ rating: 5 })}
                      >
          ...
          <Collapsible open={rawDataOpen} onOpenChange={setRawDataOpen}>
            <Card>
              ...
              <CollapsibleContent>
                <CardContent>
                  <pre
                    className="text-xs font-mono bg-muted/30 p-3 rounded-md overflow-auto max-h-96"
                    data-testid="text-raw-data"
                  >
            ...
            <Card>
              ...
              <CollapsibleContent>
                <CardContent>
                  <pre
                    className="text-xs font-mono bg-muted/30 p-3 rounded-md overflow-auto max-h-96"
                    data-testid="text-normalized-data"
                  >
                    {alert.normalizedData
                      ? JSON.stringify(alert.normalizedData, null, 2)
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {showTagSelect ? (
                ...
                <Button size="sm" variant="outline" onClick={() => setShowTagSelect(true)} data-testid="button-add-tag">
                  ...
                  {alertEntities.map((entity) => (
                    <Link key={entity.id} href="/entity-graph">
                      <div
                        ...
                        data-testid={`alert-entity-${entity.id}`}
                      >
