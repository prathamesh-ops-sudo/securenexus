...
export function GlobalQuickCreate() {
  ...
  const handleSubmitNote = useCallback(async () => {
    if (!noteContent.trim()) {
      toast({ title: "Note content required", variant: "destructive" });
      return;
    }
  ...
  const handleSubmitChannel = useCallback(async () => {
    ...
    setIsSubmitting(true);
    try {
      await apiRequest("POST", "/api/notification-channels", {
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            size="icon"
            variant="default"
            className="fixed bottom-6 right-6 z-50 h-12 w-12 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 active:scale-95"
            data-testid="button-global-quick-create"
          >
        ...
        <DropdownMenuContent align="end" side="top" className="w-56 mb-2">
          ...
          <DropdownMenuItem
            onClick={() => setMode("notification-channel")}
            data-testid="quick-create-notification-channel"
          >
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="qc-alert-id">
                Alert ID <span className="text-muted-foreground text-xs">(optional)</span>
              </Label>
              <Input
                id="qc-alert-id"
                placeholder="e.g. alert-abc123"
                value={noteAlertId}
                onChange={(e) => setNoteAlertId(e.target.value)}
                data-testid="input-qc-alert-id"
              />
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={4}
                data-testid="input-qc-note-content"
              />
              {!noteContent.trim() && noteContent.length > 0 && (
                <p className="text-xs text-destructive">Note content cannot be empty</p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              data-testid="button-qc-submit-note"
            >
              {isSubmitting ? (
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                data-testid="input-qc-task-title"
              />
              {!taskTitle.trim() && taskTitle.length > 0 && (
            ...
            <div className="space-y-2">
              ...
              <Select value={taskSeverity} onValueChange={setTaskSeverity}>
                <SelectTrigger data-testid="select-qc-severity">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              data-testid="button-qc-submit-task"
            >
      ...
      <Dialog
        open={mode === "notification-channel"}
        onOpenChange={(open) => {
          if (!open) resetForm();
        }}
      >
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                data-testid="input-qc-channel-name"
              />
              {!channelName.trim() && channelName.length > 0 && (
                <p className="text-xs text-destructive">Channel name is required</p>
              )}
            </div>
            <div className="space-y-2">
              <Label>Type</Label>
              <Select value={channelType} onValueChange={setChannelType}>
                <SelectTrigger data-testid="select-qc-channel-type">
              ...
              {!channelType && channelName.trim() && (
                <p className="text-xs text-destructive">Please select a channel type</p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!channelName.trim() || !channelType || isSubmitting}
              data-testid="button-qc-submit-channel"
            >
              {isSubmitting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Plus className="h-4 w-4 mr-2" />}
              Create Channel
            </Button>
